...
class RoidStateMachine {
  constructor() {
    ...
    this.machine.registerState(Events.SIGNED_IN, [Events.PREPARING, Events.FAILED]);
    this.machine.registerState(Events.PREPARING, [Events.WAITING_OPERATOR, Events.FAILED]);
    this.machine.registerState(Events.WAITING_OPERATOR, [Events.UNDER_CONTROL, Events.FAILED]);
    this.machine.registerState(Events.UNDER_CONTROL, [Events.WAITING_OPERATOR, Events.FAILED]);
  ...
  prepare() {
    ...
    const createConsumerPromise = channelsConnection.createConsumer(this.data.cred)
    openBoard.readyBoard(board)
    Promise.all([mqttPromise, createConsumerPromise]).then(() => {
